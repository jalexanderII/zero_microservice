...
var L = hclog.Default()
var db = userDB.InitiateMongoClient()
...
func Test_authServer_Login(t *testing.T) {
	...
	server := AuthServer{DB: db, jwtm: jwtManager, l: L}
...
func Test_authServer_SignUp(t *testing.T) {
	...
	_, err := server.SignUp(ctx, &userPB.SignupRequest{Username: "Carl", Email: "example@gmail.com", Password: "examplestring"})
	if err.Error() != "Username already taken" {
		t.Error("2: No or wrong error returned for username already taken")
	}

	_, err = server.SignUp(ctx, &userPB.SignupRequest{Username: "Carlo", Email: "test@gmail.com", Password: "examplestring"})
	...
	_, err = server.SignUp(ctx, &userPB.SignupRequest{Username: "Example", Email: "e", Password: "pass"})
